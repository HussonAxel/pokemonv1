...
import { ScrollArea, ScrollAreaPrimitive } from "#/components/ui/scroll-area.tsx";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { FileThumbnail } from "#/components/extend/file-thumbnail.tsx";
...
export function FileSystem({
  ...
  const applySortKey = React.useCallback((key: FileSystemSortKey) => {
    setSort((previous) =>
      previous.key === key ? previous : { direction: defaultSortDirection(key), key },
    );
  }, []);
  ...
  const dialogStagePath =
    openedFile !== null && openedFile.kind !== "image" ? openedFile.file.path : null;
      ...
      <div className="relative grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 border-b bg-muted/40 px-2">
        <div className="flex min-w-0 items-center gap-0.5">
          ...
          {headerLayout !== "minimal" ? (
            <span className="ml-1.5 truncate text-sm font-semibold">{currentFolderName}</span>
          ) : null}
        </div>
        {headerLayout !== "full" || isBelowIpadWidth ? (
          <Select value={view} onValueChange={(value) => setView(value as FileSystemView)}>
            ...
            >
              <SelectValue>
                {activeViewOption ? (
                  <HugeiconsIcon icon={activeViewOption.icon} className="size-4" />
                ) : null}
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
          ...
          {filters.map((filter) => {
            ...
            return (
              <FileSystemFilterPill
                ...
                filter={filter}
                onOpenCustomRange={
                  dateFilterType ? () => openDateRangeDialog(dateFilterType) : undefined
                }
                ...
                onRemove={() =>
                  setFilters((previous) => previous.filter((entry) => entry.id !== filter.id))
                }
                ...
                {groupOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      <HugeiconsIcon
                        icon={Tick02Icon}
                        className={cn("size-4 text-foreground", !isChecked && "opacity-0")}
                      />
...
function FileSystemFilterPill({
  ...
  const selectedTypeLabels =
    filter.type === "fileType"
      ? filter.value.map(
          (mime) => fileTypeOptions.find((option) => option.mime === mime)?.label ?? mime,
        )
      : [];
  ...
  return (
    <div className="flex items-center text-xs">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button type="button" className={cn(FILTER_PILL_BUTTON_CLASSNAME, "text-primary")}>
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-y-px">
                {cells.map((day, cellIndex) => {
                  ...
                  const isWithinRange =
                    fromKey !== null && toKey !== null && dayKey > fromKey && dayKey < toKey;
...
function useEntryTypeAhead() {
  ...
  return React.useCallback(
    (event: React.KeyboardEvent, entries: readonly FileSystemEntry[], currentIndex: number) => {
...
function moveGridSelection({
  ...
  if (currentIndex === -1) {
    ...
    for (const entry of entries) {
      ...
      const rowDelta =
        key === "ArrowDown" ? rect.top - currentRect.top : currentRect.top - rect.top;
  ...
  React.useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      setColumnCount(
        Math.max(
          1,
          Math.floor((available + ICON_TILE_GAP_X) / (ICON_MIN_TILE_WIDTH + ICON_TILE_GAP_X)),
        ),
      );
    };
...
function FileSystemListView({
  ...
  if (relativePaths.length === 0) {
    return (
      <FileSystemEmptyState
        label={fileFilter ? "No items match the active filters" : "This folder is empty"}
      />
    );
  }
  ...
  const { model } = useFileTree({
    ...
    renderRowDecoration: ({ row }) => {
      ...
      if (entry.kind === "folder") {
        ...
        return {
          text:
            childCount === undefined ? "—" : `${childCount} ${childCount === 1 ? "item" : "items"}`,
          title: dateColumn,
        };
      }
...
function FileSystemColumnsView(props: FileSystemViewProps) {
  ...
  const selectedFile =
    deferredSelectedEntry?.kind === "file" ? (deferredSelectedEntry as FileEntry) : null;
      ...
      >
        {columnPaths.map((columnPath, columnIndex) => (
          <FileSystemColumn
            ...
            selectedChildPath={
              selectedPath && pathParent(selectedPath) === columnPath ? selectedPath : null
            }
            tabStopChildPath={
              tabStopPath && pathParent(tabStopPath) === columnPath ? tabStopPath : null
            }
...
function FileSystemGalleryView(props: FileSystemViewProps) {
    ...
    () =>
      new Map(
        attachedStagePaths.map(
          (path) =>
            [path, (element: HTMLElement | null) => registerStageHost(path, element)] as const,
        ),
      ),
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    const nextEntry =
      entries[currentIndex === -1 ? 0 : currentIndex + (event.key === "ArrowLeft" ? -1 : 1)];
