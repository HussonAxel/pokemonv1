          ...
          {displayUsers.length > 0 && variant !== "ghost" && (
            <div aria-hidden="true" className={cn("flex items-center", sizeConfig.avatarStack)}>
              {displayUsers.map((user, index) => (
                ...
                >
                  <Avatar
                    className={cn(sizeConfig.avatar, "border border-background ring-1 ring-border")}
                  >
                    <AvatarImage
        ...
        <AnimatePresence>
          {isOpen && loadedUsers.length > 0 && (
            ...
            >
              ...
              <div className="max-h-[140px] max-w-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
                <ul
                  className="flex flex-wrap items-center gap-x-2 gap-y-1.5 px-1"
                  aria-label="People who liked this"
                >
                  {visibleUsersInPopup.slice(0, 6).map((user, index) => (
                    <motion.li
                      key={user.id}
                      ...
                      className="group inline-flex w-fit items-center gap-1.5 whitespace-nowrap py-0.5"
                    >
                      <Avatar
                        className={cn(sizeConfig.popupAvatar, "shrink-0 border border-border")}
                      >
                        <AvatarImage
                          src={user.avatar || "/placeholder.svg"}
                          alt=""
                      ...
                      <span className="text-xs text-foreground/80 transition-colors group-hover:text-foreground">
                        {user.name}
                        {index < Math.min(visibleUsersInPopup.length, 3) - 1 ? "," : "..."}
                      </span>
                    </motion.li>
                  ))}
                </ul>
              ...
              <div className="flex items-center justify-between mt-4 mb-2 px-1">
                <span className="text-xs font-medium text-muted-foreground">Hunted by</span>
                <span className="text-xs tabular-nums text-muted-foreground">
                  {localCount.toLocaleString()}
                </span>
              </div>

              <div className="max-h-[140px] max-w-[400px] overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
                <ul
                  className="flex w-full max-w-[450px] flex-wrap items-center justify-start gap-x-2.5 gap-y-1.5 px-1 text-left"
                  aria-label="People hunting this"
                >
                  {visibleUsersInPopup.slice(0, 6).map((user, index) => (
                    <motion.li
                      key={user.id}
                      ...
                      className="group inline-flex w-fit shrink-0 items-center gap-1.5 whitespace-nowrap py-0.5"
                    >
                      <Avatar
                        className={cn(sizeConfig.popupAvatar, "shrink-0 border border-border")}
                      >
                      ...
                      <span className="text-xs leading-5 text-foreground/80 transition-colors group-hover:text-foreground">
                        {user.name}
                        {index < Math.min(visibleUsersInPopup.length, 3) - 1 ? "," : "..."}
                      </span>
                    </motion.li>
                  ...
                  {onLoadMore && canLoadMore ? (
                    ...
                    >
                      {isLoadingMore ? (
                        ...
                        <span>
                          Load more{" "}
                          {totalRemaining > 0 && `(${totalRemaining.toLocaleString()} more)`}
                        </span>
                      )}
                    </button>
                  ) : totalRemaining > 0 ? (
