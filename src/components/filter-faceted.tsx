...
const ISSUES: Issue[] = [
  ...
  {
    id: "ENG-211",
    title: "Investigate stripe.charge.failed spike",
    status: "todo",
    priority: "p0",
    assignee: "ada",
    labels: ["bug"],
  ...
  {
    id: "ENG-210",
    title: "Schema validation: missing event.id",
    status: "in-progress",
    priority: "p1",
    assignee: "linus",
    labels: ["bug", "infra"],
  ...
  {
    id: "DOC-44",
    title: "Document new webhook signing flow",
    status: "todo",
    priority: "p3",
    assignee: "grace",
    labels: ["docs"],
  ...
  {
    id: "ENG-209",
    title: "Rate limit handling for github oauth",
    status: "todo",
    priority: "p2",
    assignee: "alan",
    labels: ["infra"],
  ...
  {
    id: "ENG-201",
    title: "Remove deprecated team_id query param",
    status: "in-review",
    priority: "p2",
    assignee: "ada",
    labels: ["feature"],
  ...
  {
    id: "ENG-200",
    title: "Mobile push notifications backend",
    status: "in-progress",
    priority: "p1",
    assignee: "linus",
    labels: ["feature"],
  ...
  {
    id: "ENG-189",
    title: "Postgres → planetscale prep work",
    status: "todo",
    priority: "p1",
    assignee: "alan",
    labels: ["infra"],
  ...
  {
    id: "DSN-83",
    title: "Pricing page A/B variant B",
    status: "in-review",
    priority: "p2",
    assignee: "grace",
  ...
  {
    id: "ENG-184",
    title: "Cache projects/atlas (TTL 12s)",
    status: "done",
    priority: "p3",
    assignee: "linus",
    labels: ["infra"],
  ...
  {
    id: "ENG-180",
    title: "Combobox keyboard focus regression",
    status: "todo",
    priority: "p2",
    assignee: "ada",
    labels: ["bug"],
  ...
  {
    id: "ENG-176",
    title: "Toast double-announce in Safari",
    status: "done",
    priority: "p3",
    assignee: "linus",
    labels: ["bug"],
  ...
  {
    id: "DOC-40",
    title: "Rewrite onboarding tutorial",
    status: "in-progress",
    priority: "p2",
    assignee: "grace",
    labels: ["docs"],
  ...
  {
    id: "DSN-79",
    title: "Card: stat tile sparklines",
    status: "done",
    priority: "p3",
    assignee: "grace",
    labels: ["design", "feature"],
...
export function FilterFacetedShowcasePage() {
  ...
  const counts = useMemo(() => {
    const count = <T extends string>(facet: T, get: (i: Issue) => T | T[]): number =>
      ISSUES.filter((i) => {
        const v = get(i);
  ...
  const totalSelected = status.length + priority.length + assignee.length + label.length;
  const reset = () => {
    setStatus([]);
    setPriority([]);
    setAssignee([]);
        ...
        <div className="rounded-xl border bg-card p-4 shadow-xs/5">
          <div className="flex flex-wrap items-center gap-2">
            <FacetTrigger
              ...
              onClear={() => setStatus([])}
            >
              <FacetCheckboxList value={status} onValueChange={(v) => setStatus(v as StatusKey[])}>
                {STATUS_FACETS.map((f) => {
                  const Icon = f.icon;
                  return (
                    <FacetRow
            ...
            <FacetTrigger
              ...
              onClear={() => setLabel([])}
            >
              <FacetCheckboxList value={label} onValueChange={(v) => setLabel(v as LabelKey[])}>
                {LABEL_FACETS.map((f) => (
                  <FacetRow
                    key={f.value}
                    value={f.value}
            ...
            <Popover>
              ...
              <PopoverPopup className="w-56 p-1">
                ...
                <p className="px-2 py-2 text-muted-foreground text-xs">
                  Custom fields aren't wired up in this demo — open the facets above instead.
                </p>
              </PopoverPopup>
            </Popover>
          </div>
          ...
          <div className="flex items-center justify-between text-muted-foreground text-xs">
            ...
            <Button size="xs" variant="ghost" onClick={reset} disabled={totalSelected === 0}>
              Reset filters
            </Button>
          </div>
        </div>
      ...
      <PopoverTrigger
        render={
          <Button
            size="sm"
            variant="outline"
            className={count > 0 ? "border-foreground/20 bg-foreground/5" : "border-dashed"}
          >
            {icon ?? <PlusCircleIcon />}
            {label}
            {count > 0 ? (
...
function FacetRow({
  ...
  return (
    <Label className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-accent">
      ...
      {leading ? <span className="flex size-5 items-center justify-center">{leading}</span> : null}
...
function IssueList({ issues, onReset }: { issues: Issue[]; onReset: () => void }) {
  if (issues.length === 0) {
    return (
      <div className="mt-6 rounded-xl border border-dashed bg-card/40 p-12 text-center">
        <p className="text-muted-foreground text-sm">No issues match these filters.</p>
